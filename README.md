...
|-----------------|-----------------------------------|---------------------------------|
| `JWT_SECRET_KEY`| `change-me-in-production-please`  | Секрет для подписи JWT-токенов  |
| `DB_PATH`       | `db.sqlite`                       | Путь к файлу базы данных        |
| `PORT`          | `5000`                            | Порт сервера                    |

В продакшене обязательно задайте `JWT_SECRET_KEY`:
```bash
